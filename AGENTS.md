...
- **Contact custom fields:** Event Logs `7D9N71mEDfipN90zfV0j` · Call Summary `ZVeEoK85i5EOhWt1HO1F` ·
  ...
  **Call Router Context** `HW0eBfoQPW2mwxX8aY7Q` (JSON, write-once per call by Call-Disposition Automation 1 — `{opp_id, route, caller_N, call_id, stage_name, stage_id, call_duration, call_recording_url}`. **On an anomaly Automation 1 blanks this field and Call Processing State instead of writing them** — Automation 2 is trigger-by-field-change and still fires on the disposition, so a leftover ctx would make it move the *previous* call's opp; empty ⇒ `route='none'` ⇒ no handler; `stage_id` is the raw caller-stage id the Cold Handler's voicemail branch needs for its missed-call-email `mc` lookup. `call_duration`/`call_recording_url` are **metrics passengers** — nothing routes on them; they live here because the call-recorded webhook is the only place they ever appear, and the handlers need them for the `call_log` row) ·
  ...
  **Stop Emails** `ixRO9dSUHVd6vNTdFa7Q` (**radio**, `True`/`False` — `True` ⇒ **no email is sent**: the scheduled senders (`Send Cold Email 1`, `Send Cold Email 2/3/4`) and both Cold Handlers move the opp **straight to `Cold Email N Sent`** instead of enrolling in Instantly, and the missed-call/voicemail email is skipped. **The Cold/Gatekeeper + missed-call handlers also treat a contact with no `email` address as skip-email** (empty email ⇒ same as `Stop Emails=True`): they route it to `Cold Email N Sent` and skip every Instantly/missed-call-email call, so a no-email lead dropped straight into `Day 1 Call A` still advances through the `Sent` stages instead of stalling. (The **scheduled senders** `Send Cold Email 1/2/3/4` still key on the field alone — a no-email lead entering the *email* pipeline should carry `Stop Emails=True` or use the `No Email Cold Call 1` stage.)
...
- **`screener_sweep_pending`** `J61RThPMfxykZt1N` — contacts whose Daily Sweep writes half failed (all their writes as
  `ops_json`); the next sweep replays them unless the lead moved since. Normally empty.
...
- **Writers:** `Screener: Compare Step` (answered calls, key `call:<call_id>`) and `Screener: Attempt
  Counter` (no-answer / voicemail / bad-number, the ladder's `event_key`), always through the function
  **`screener_log_upsert`** (atomic, versioned on `decided_ms` = when the run read GHL: a decision is
  replaced only by a same-or-later read; call facts only filled in). `onError: continue`; a failed write
  is queued in n8n table **`screener_log_pending`** `qKv7RxgTDsqb1plo` and replayed by
  **`Screener: Log Retry`** `y2oXfZtH4y1mhWQG` (Supabase only, never GHL). Project URL + n8n credential live in
...
- **Status:** table + view created (migration `create_screener_log`; RLS on, no policies
  ...
  Obby bot `QcTNBiXBrnH5rFkC`). Since 2026-09-26 it also has Kevin's 7 call columns (`shop_id`, `number_dialed`,
  `how_he_answered`, `voicemail_greeting`, `first_name`, `best_time`, `background`; n8n doesn't fill them yet) and
  `fields_ms`; current function: `supabase/screener_log_codex_fixes.sql`. Kevin reads it through the view `calls`.

## Supabase database — Kevin's schema (live since 2026-09-26)
History: our first design (`shops`/`shop_raw`/`call_log`/`transcripts`, migration `create_core_tables` 2026-09-25,
[`supabase/core_tables.sql`](supabase/core_tables.sql), [`docs/supabase-design.md`](docs/supabase-design.md)) never held
data and now sits in schema `archive`. Rules that still hold: Supabase keeps facts, GHL keeps state (no two-way sync);
logs join `shops` on `ghl_contact_id`; RLS on everywhere, service role only. Free plan (500 MB; ~54 MB used; no backups;
pauses after 7 idle days): Pro later, weekly backup meanwhile.
