...
- **Contact custom fields:** Event Logs `7D9N71mEDfipN90zfV0j` · Call Summary `ZVeEoK85i5EOhWt1HO1F` ·
  ...
  **Only `Email Sent → Move To Sent Stage` writes it** — dropped from `missed-call/Cold Handler` 2026-07-14) ·
...
- **Retired — do not reintroduce** (all removed 2026-07-14):
  `lifecycle_status` (from `Send Cold Email 1`) ·
  `JuNrjuwlZXMkJZ9R7qsi`, was written as `cold call N` (from `Sent Cold Email to Caller Stages`) ·
  **`lifecycle` `9T7lODjinSThOnAzAI0G`** (from `Email Sent → Move To Sent Stage`, where it gated
  the stage move — routing now derives from `email_step_name`; also dropped from
  `missed-call/Cold Handler`, where it was declared but never used).
  **`lifecycle` is now referenced by zero workflows** — safe to delete in GHL once the unmirrored
  automations are confirmed clear.
...
_Dispatcher route `cold`. **Restructured 2026-07-14** — full current stage list (15 stages)._
...
- **`(from on hold)`** = resumed from an on-hold state. _Semantics TBD — no workflow targets these yet._

> 🚨 **The restructure REUSED 6 old stage IDs under new names.** Same-pipeline rename +
> expansion, **not** a new pipeline. The old IDs still resolve, so **nothing errors** — but what
> they *mean* changed. See **[Migration impact](#migration-impact-cold-call-restructure)**.

| Stage | ID | Note |
|---|---|---|
| Day 1 Call A | `060f44a8-4cd8-4561-8c84-7150bfd57498` | ♻️ was `Cold Call 1` |
| Day 1 Call A (MGR) | `04ccf8e0-383f-4c17-9268-51020233d08b` | ♻️ was `Cold Call 1 - Missed Call (Google Review)` |
| Day 1 Call A (from on hold) | `cb2dc70f-c44e-48fd-b9cc-438901e292ac` | 🆕 |
| Day 1 Call B | `4cb90aaa-b8c3-40a8-b1e6-6585213b258c` | ♻️ was `Cold Call 2` |
| Day 1 Call B (MGR) | `648c4952-1ac0-4c8c-9e52-b93efa5ec39e` | ♻️ was `Cold Call 2 - Missed Call` |
| Day 2 Call | `4b1d7a88-87c0-422b-90e4-b48d16430900` | ♻️ was `Cold Call 3` |
| Day 2 Call (MGR) | `4b3023aa-515b-40f8-ad82-292ca685562e` | ♻️ was `Cold Call 3 - Missed Call` |
...
**♻️ = old ID, new name (6) · 🆕 = brand-new ID (9). No old stage ID was dropped.**

Note `MGR` is a **separate axis from `missed call`** (Day 2/3 have both `(MGR)` and
`MGR (missed call)` variants) — so `MGR` is *not* simply the old "Missed Call". Two of the
...
**1. `STAGE_TO_N` no longer means "day".** Both Cold Handlers + the call Dispatcher hardcode:

| ID | mapped N | old name | **new name** |
|---|---|---|---|
| `060f44a8…` / `04ccf8e0…` | **1** | Cold Call 1 / CC1-Missed | Day 1 Call **A** / A (MGR) |
| `4cb90aaa…` / `648c4952…` | **2** | Cold Call 2 / CC2-Missed | Day 1 Call **B** / B (MGR) |
| `4b1d7a88…` / `4b3023aa…` | **3** | Cold Call 3 / CC3-Missed | Day **2** Call / (MGR) |

So `N` is an **attempt index**, and it has drifted out of sync with the day: Day 1 now holds
**two** attempts (A=1, B=2), and Day 2 is N=3. `N` and "day" are no longer the same number —
anything that reasons about `N` as a day is now wrong. This also feeds `SEND_NEXT`
(N→Cold Email N+1), so the email step a call outcome triggers is affected.

**2. The 9 new stages are invisible to every workflow.** None are in `STAGE_TO_N`, so they
resolve to `N=0` → `routable=false` → **leads parked in any Day 3 stage, any `(from on hold)`
...
### Missed-call semantics (post-migration)
...
## `GHL Pipeline Stages (Cached)` — being phased out
The `ny7jwqGX1Du9aXNC` sub-workflow supplied pipeline/stage/custom-field IDs at runtime. It is
**being replaced by inlined literals**, workflow by workflow:

| Workflow | Cached Values |
|---|---|
| `call-disposition/Dispatcher` | ✅ already inlined |
| `scheduled-automations/Send Cold Email 1` | ✅ removed 2026-07-14 |
| `missed-call/Dispatcher` | ✅ removed 2026-07-14 |

**As of 2026-07-14, no mirrored workflow calls it.** It is still marked Active in n8n, so
unmirrored workflows may still depend on it — **confirm before archiving it.** When inlining,
take IDs from the reference tables in this file rather than re-deriving them.

## ⚠️ Open questions
- 🚨 **Cold-call pipeline was restructured (2026-07-14) and no workflow has been migrated.**
  6 old stage IDs were reused under new names, so live workflows still run but apply stale
  semantics; 9 new stages are invisible to every workflow. See
  [Migration impact](#migration-impact-cold-call-restructure). **Needs:** stage semantics
  (`MGR`? `from on hold`? `missed call`?) and the intended `stage → N` mapping.
- **The email sequence has no entry point right now.** `Send Cold Email 2/3/4` is **Active**, but
  both step-1 workflows are **Inactive** — steps 2/3/4 run nightly while step 1 never fires.
  Confirm whether email 1 is intentionally paused.
- ✅ ~~What moves a lead `Cold Email N` → `Cold Email N Sent`?~~ **Answered 2026-07-14:**
  [`Email Sent → Move To Sent Stage`](workflows/email-sent/) (`CDdLps7wfOjyM9Lx`), an
  Instantly webhook. Now mirrored.
