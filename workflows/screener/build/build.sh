...
set -e
cd "$(dirname "$0")"
node gen_classifier.js >/dev/null && node gen_capture.js && node gen_eval.js && node gen_compare.js && node gen_mark.js && node gen_retry.js && node gen_testrig.js && node gen_counter.js && node gen_eventhooks.js && node gen_graduate.js && node gen_logretry.js && node snapshot.js
