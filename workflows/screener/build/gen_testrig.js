...
const gradRow = node({
  ...
  config: { name: 'Test contact graduation', position: [224, 300], alwaysOutputData: true, parameters: { resource: 'row', operation: 'get',
    dataTableId: { __rl: true, mode: 'id', value: '1iX0aTvMYawwyH4H', cachedResultName: 'screener_graduations' }, matchType: 'allConditions',
    filters: { conditions: [ { keyName: 'grad_key', condition: 'eq', keyValue: '2Z5mwZe5RT4NQdNW85vj:FAstcBVvrgbpds2gQIV3' } ] }, returnAll: false, limit: 1 } },
  output: [{ id: 1, grad_key: '2Z5mwZe5RT4NQdNW85vj:FAstcBVvrgbpds2gQIV3', kevin_opp_id: 'K1', created_new: true }]
});

const rigOps = node({
  type: 'n8n-nodes-base.code', version: 2,
  config: { name: 'Rig Ops', position: [448, 300], parameters: { mode: 'runOnceForEachItem', jsCode: ${c('test_rig_ops.js')} } },
...
const hasOps = ifElse({
  version: 2.2,
  config: { name: 'Anything to write?', position: [672, 300], parameters: { conditions: {
...
const split = node({
  type: 'n8n-nodes-base.code', version: 2,
  config: { name: 'Split Rig Ops', position: [896, 208], parameters: { mode: 'runOnceForAllItems', jsCode: "return $('Rig Ops').first().json.ops.map(o => ({ json: o }));\\n" } },
  output: [{ label: 'x', method: 'PUT', url: 'https://services.leadconnectorhq.com/contacts/2Z5mwZe5RT4NQdNW85vj', body: {} }]
});

const apply = node({
  type: 'n8n-nodes-base.httpRequest', version: 4.4,
  config: { name: 'GHL: Rig Apply', position: [1120, 208], onError: 'continueRegularOutput',
...
const once = node({
  type: 'n8n-nodes-base.code', version: 2,
  config: { name: 'Then read back', position: [1344, 300], parameters: { mode: 'runOnceForAllItems', jsCode: "return [{ json: {} }];\\n" } },
  output: [{}]
});

const readContact = node({
  type: 'n8n-nodes-base.httpRequest', version: 4.4,
  config: { name: 'GHL: Read Contact', position: [1568, 300], onError: 'continueRegularOutput',
...
const readOpp = node({
  type: 'n8n-nodes-base.httpRequest', version: 4.4,
  config: { name: 'GHL: Read Opp', position: [1792, 300], onError: 'continueRegularOutput',
...
const state = node({
  type: 'n8n-nodes-base.code', version: 2,
  config: { name: 'State', position: [2016, 300], parameters: { mode: 'runOnceForAllItems', jsCode: ${c('test_rig_state.js')} } },
  output: [{ action: 'read', stage: 'Attempt 1', followers: [], tags: ['screening'], fields: {} }]
});

const note = sticky('## Screener: Test Rig  (manual only, never activate)\\nHard-wired to the team test contact Dana Happy 2Z5mwZe5RT4NQdNW85vj and its screener opp FAstcBVvrgbpds2gQIV3. Body action: read (state only), mark (outcome: plays the screener by setting Screener Outcome), reset (clears the five screener fields incl. Screen Attempts, result tags, block followers; screening tag back; opp open in Attempt 1), ungraduate (deletes only the Kevin opp that Screener: Graduate logged as created for Dana, then reset). Do Not Call is refused: it would set DND on the receptionist demo record.', [rigOps, apply, state], { color: 3 });

export default workflow('screener-test-rig', 'Screener: Test Rig')
  .add(hook).to(gradRow).to(rigOps)
